...
using System.Text;
using BeatmapParser.Enums.Storyboard.Command;

namespace BeatmapParser.Events.Commands;
...
public class Colour : ICommand
{
    ...
    public static Colour Decode(string line)
    {
        ...
        Color? startColour = commandSplit.Length > 4 && !string.IsNullOrEmpty(commandSplit[4]) ? Helpers.Helper.ParseColorFromUnknownString(string.Join(',', commandSplit[4], commandSplit[5], commandSplit[6])) : null;
        Color? endColour = commandSplit.Length > 7 && !string.IsNullOrEmpty(commandSplit[7]) ? Helpers.Helper.ParseColorFromUnknownString(string.Join(',', commandSplit[7], commandSplit[8], commandSplit[9])) : null;
