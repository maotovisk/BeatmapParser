...
public class Slider : HitObject
{
    ...
    /// </summary>
    public (HitSounds.HitSample SampleData, List<HitSound> Sounds) HeadSounds { get; set; }

    /// <summary>
    /// 
    /// </summary>
    public List<(HitSounds.HitSample SampleData, List<HitSound> Sounds)>? RepeatSounds { get; set; }


    /// <summary>
    /// Hit sound of the slider track.
    /// </summary>
    public (HitSounds.HitSample SampleData, List<HitSound> Sounds) TailSounds { get; set; }
    ...
    public Slider(Vector2 coordinates, TimeSpan time, HitObjectType type, (HitSounds.HitSample, List<HitSound>) hitSounds, bool newCombo, uint comboOffset, List<CurvePoint> controlPoints, uint slides, double length, TimeSpan endTime, (HitSounds.HitSample SampleData, List<HitSound> Sounds) headSounds, List<(HitSounds.HitSample SampleData, List<HitSound> Sounds)>? repeatSounds, (HitSounds.HitSample SampleData, List<HitSound> Sounds) tailSounds) :
    ...
    public Slider()
    {
        ...
        HeadSounds = (new HitSounds.HitSample(), []);
        RepeatSounds = [];
        TailSounds = (new HitSounds.HitSample(), []);
    ...
    /// <param name="difficultySection">The difficulty.</param>
    /// <returns>The parsed slider hit object.</returns>
    public static Slider Decode(List<string> split, TimingPointsSection timingPoints, DifficultySection difficultySection)
    {
        try
        {
            ...
            if (objectParams.Length > 0 && !string.IsNullOrEmpty(objectParams[0]) && char.IsLetter(objectParams[0][0]))
            {
                ...
                if (Helpers.Helper.FormatVersion == 14)
                {
            ...
            for (var i = startIndex; i < objectParams.Length; i++)
            {
            ...
            return new Slider(baseObject)
            {
                ...
                TailSounds = sliderHitSounds.Count == 0 ? (new HitSounds.HitSample(), new List<HitSound>()) : sliderHitSounds.Last(),
            };
        }
        catch (Exception ex)
        {
    ...
    public new string Encode()
    {
        ...
        if (Helpers.Helper.FormatVersion == 14)
        {
            ...
            pathBuilder.Append(Helpers.Helper.EncodeCurveType(legacy));
            foreach (var cp in ControlPoints)
            {
                pathBuilder.Append('|');
                pathBuilder.Append($"{Helpers.Helper.FormatCoord(cp.Position.X)}:{Helpers.Helper.FormatCoord(cp.Position.Y)}");
            }
        }
        else
        {
            ...
            {
                pathBuilder.Append(Helpers.Helper.EncodeCurveType(LegacyCurveType.Value));
                pathBuilder.Append('|');
            }
            ...
            {
                pathBuilder.Insert(0, Helpers.Helper.EncodeCurveType(LegacyCurveType ?? CurveType.Bezier) + "|");
            }
        }
        ...
        bool hasNonDefaultSampleData = new[] { HitSounds.SampleData, HeadSounds.SampleData, TailSounds.SampleData }
            .Concat(RepeatSounds?.Select(x => x.SampleData) ?? Enumerable.Empty<HitSounds.HitSample>())
        ...
        {
            edgeSounds.Add(Helpers.Helper.EncodeHitSounds(HeadSounds.Sounds));
        }

        if (RepeatSounds != null && RepeatSounds.Any(x => x.Sounds.Count > 0) ||
            TailSounds.Sounds.Count > 0)
        {
            edgeSounds.AddRange(RepeatSounds?.Select(x => Helpers.Helper.EncodeHitSounds(x.Sounds))
                              ?? Enumerable.Repeat(0, (int)Slides - 1));
        }

        if (TailSounds.Sounds.Count > 0)
        {
            edgeSounds.Add(Helpers.Helper.EncodeHitSounds(TailSounds.Sounds));
        }
    ...
    /// </summary>
    private static bool HasNonDefaultSampleData(HitSounds.HitSample sample) =>
