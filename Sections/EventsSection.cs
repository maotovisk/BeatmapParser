...
public class EventsSection
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="EventsSection"/> class.
    /// </summary>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="EventsSection"/> class with the specified parameters.
    /// </summary>
    /// <param name="eventList"></param>
    public EventsSection(List<IEvent> eventList)
    {
        EventList = eventList;
    }
    ...
    /// <summary>
    /// Converts a list of strings to a <see cref="EventsSection"/> object.
    /// </summary>
    /// <param name="sectionRaw"></param>
    /// <returns></returns>
    public static EventsSection Decode(List<string> sectionRaw)
    {
        try
        {
            ...
            for (var index = 0; index < section.Count; ++index)
            {
                ...
                if (depth == 0)
                {
                    ...
                    IEvent? @event = Helpers.Helper.ParseEventType(eventSplit) switch
                    {
    ...
    private static int AddCommands(IHasCommands currentEvent, List<string> sectionRaw, int index, int depth)
    {
        ...
        while (tempindex < sectionRaw.Count)
        {
            ...
            if (currentdepth == depth)
            {
                currentEvent.Commands.Add(Helpers.Helper.ParseCommand(sectionRaw[tempindex]));
                ++tempindex;
            }
        }
        return tempindex;
    ...
    /// <summary>
    /// Encodes the <see cref="EventsSection"/> section to a string.
    /// </summary>
    /// <returns></returns>
