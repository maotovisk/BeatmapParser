...
public class EditorSection
{
    ...
    /// <summary>
    /// Constructs a new instance of the <see cref="EditorSection"/> class.
    /// </summary>
    ...
    private EditorSection(List<double>? bookmarks, double distanceSpacing, int beatDivisor, int gridSize, double? timelineZoom)
    {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="EditorSection"/> class.
    /// </summary>
    public EditorSection() { }

    /// <summary>
    /// Converts a list of strings to a <see cref="EditorSection"/> object.
    /// </summary>
    /// <param name="sections"></param>
    /// <returns></returns>
    public static EditorSection Decode(List<string> sections)
    {
    ...
    /// <summary>
    /// Encodes the <see cref="EditorSection"/> section into a string.
    /// </summary>
    /// <returns></returns>
    public string Encode()
    {
        ...
        foreach (var prop in typeof(EditorSection).GetProperties())
        {
            if (prop.GetValue(this) is null) continue;
            if (prop.GetValue(this) is string str && string.IsNullOrEmpty(str) && Helpers.Helper.FormatVersion == 128) continue;
            ...
            {
                builder.AppendLine($"{prop.Name}: {string.Join(',', _bookmarkMilliseconds.Select(Helpers.Helper.FormatTime))}");
                continue;
            }
