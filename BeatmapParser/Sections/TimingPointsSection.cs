...
public class TimingPointsSection
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="TimingPointsSection"/> class.
    /// </summary>
    /// <param name="timingPointList"></param>
    private TimingPointsSection(List<TimingPoint> timingPointList)
    {
    ...
    /// <summary>
    /// Parses a list of TimingPoints lines into a new <see cref="TimingPointsSection"/> class.
    /// </summary>
    /// <param name="section">The raw lines from the [TimingPoints] section of an osu file.</param>
    /// <returns>A populated <see cref="TimingPointsSection"/> instance containing the parsed timing point objects.</returns>
    public static TimingPointsSection Decode(List<string> section)
    {
        ...
        try
        {
            foreach (var line in section)
            {
                ...
                var clampedTime = Helpers.Helper.ClampMilliseconds(time);
                ...
                var effects = split.Length >= 8 ? Helpers.Helper.ParseEffects(int.Parse(split[7])) : [];
            ...
            return new TimingPointsSection(timingPoints);
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Encodes the <see cref="TimingPointsSection"/> class into a string.
    /// </summary>
    ...
    /// <returns>The latest <see cref="TimingPoints.UninheritedTimingPoint"/> whose timestamp is less than or equal to the provided time; otherwise null if none exist.</returns>
    ...
    /// <param name="time">The time in milliseconds.</param>
    /// <returns>
    /// The matching <see cref="TimingPointsSection.InheritedTimingPoint"/> if one is active at the provided time and is valid relative to the last uninherited timing point; otherwise null.
    /// </returns>
    ...
    /// </summary>
    /// <param name="time">Time in milliseconds.</param>
    /// <returns>A list of matching <see cref="TimingPointsSection.TimingPoint"/> objects. Returns an empty list if none match.</returns>
